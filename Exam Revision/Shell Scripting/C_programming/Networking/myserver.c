...
#define DEBUG // constant for debugging
...
int main(int argc, char *argv[])
{
	...
	// stuff for shutting dow nthe server
	int receivedPw = FALSE;
				...
				if (currentSock[events[i].data.fd].sockStatus == FREE){
					...
					while (readStatus != TRUE){
						...
						if (dataRead == -1){
							...
							if (clientAction == CLOSE_SERVER){ // close the server safely
								receivedPw = FALSE;
								memset(buffer, 0, sizeof(buffer));
								while (!receivedPw){
									bytesRecv = recv(events[i].data.fd, buffer, sizeof(buffer), 0);
									if (bytesRecv == -1){
										if (errno != EAGAIN){
											perror("recv(): ");
								...
								FILE *pwfd = fopen(PW_DIR, "rb");
								memset(password, 0, MAX_SIZE);
								if (pwfd != NULL){
									fgets(password, MAX_SIZE, pwfd);
									nl = strchr(password, NEW_LINE);
									if (nl != NULL) { *nl = NULL_CHAR; }
									#ifdef DEBUG
										printf("Pw: %s and buffer: %s\n", password, buffer);
									#endif
									if (strcmp(password, buffer) == 0){ // password equal so shut down the server
										closeServer = TRUE;
										endOfConnection = TRUE;
									}				
									...
									fclose(pwfd);
									pwfd = NULL;
									result = htonl(closeServer);
									if (send(events[i].data.fd, &result, sizeof(int), 0) <= 0){
										perror("send(): ");
									}
							...
							else if (clientAction != INVALID_INPUT){
