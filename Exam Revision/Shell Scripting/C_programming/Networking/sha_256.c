...
#define READ 2048000 // 2048 kb/read
...
void hashFile(const char *directory, const int fileSize, unsigned char *ourHash)
{
	int bytesRead = 0;
	int total = 0;
	sha256_context *sha256 = malloc(sizeof(sha256_context));
	FILE *fp = fopen(directory, "rb"); // open the file for reading
	char buffer[READ]; 
	...
	while (total < fileSize){
		...
		bytesRead = fread(buffer, sizeof(buffer[0]), READ - 1, fp);
		if (bytesRead <= 0){ // detect reading errors
			perror("fread(): ");
			break;
		}

		sha256_update(sha256, buffer, bytesRead); // update the hash
		total += bytesRead;
	}

	sha256_finish(sha256, ourHash); // finialise the hash
	sha256_tohex(sha256, ourHash);
	memset(ourHash, 0, HASH_SIZE);
	strncpy(ourHash, sha256->buffer, HASH_SIZE - 1); // copy the hash and return it, last element should be NULL_CHAR
